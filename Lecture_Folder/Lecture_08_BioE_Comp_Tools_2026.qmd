...
-   [Ch. 16: High-Performance Computing with Talapas](../book/chapters/16-hpc-talapas.html)
-   [Ch. 17: Parallel Computing in R](../book/chapters/17-parallel-computing.html)
...
-   You'll use SSH keys to connect to **Talapas** (today) and **GitHub** (Lecture 10)
-   Create one now with a single command: `ssh-keygen -t ed25519` (accept the defaults; a passphrase is optional but wise)
:::

::: callout-note
Your public key is `~/.ssh/id_ed25519.pub`; `ssh-copy-id you@login.talapas.uoregon.edu` puts it on Talapas so that later logins skip the password (Duo still applies).
:::
...
## Running example: the stickleback summary as a batch job

Put the Week 7 code in a script, `stickle_means.R`, that writes `results/gene_means.tsv`, then wrap it in a Slurm script:
...
::: callout-tip
Nothing about the R script changed between your laptop and Talapas - only *where* it runs and *who* launches it (`sbatch` instead of you). For the term project, this same pattern scales to files thousands of times larger.
:::
