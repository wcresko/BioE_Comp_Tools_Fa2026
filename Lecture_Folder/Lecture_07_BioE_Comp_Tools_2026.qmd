...
-   [Ch. 14: Writing Functions in R](../book/chapters/14-writing-functions.html)
...
::: callout-tip
When an object surprises you, run `str()` (R) or `.info()` / `type()` (Python) on it first. In **Positron**, the Variables pane shows the same information.
:::
...
::: callout-tip
`facet_wrap(~ group)` splits any plot into one panel per group, and `ggsave("figure.pdf")` writes the last plot to a file. Data-visualization mechanics get a full chapter in the [course book](../book/chapters/13-data-visualization.html).
:::
