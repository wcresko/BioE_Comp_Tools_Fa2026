...
| 10:50 - 11:00 | *Break*                                                            |                                      |
| 11:00 - 11:50 | **Part 3** - Your editor and the UO computing ecosystem            | R, Python, Positron                  |
| 11:50 - 12:00 | *Break*                                                            |                                      |
...
Two problem sets, each assigned two weeks before it is due (HW1 due Nov 16, HW2 due Nov 30), and a term project on Talapas (one-page proposal due Nov 9; project due Dec 11). Full details on the [Schedule](../Schedule.html) and [Syllabus](../Syllabus.html) pages.
:::
:::::
...
flowchart LR
  ...
  subgraph C["Computers & the shell"]
    direction TB
    L1["01 Computer systems,<br/>local / cluster / cloud"] --> L2["02 Files, pipes,<br/>tidy data"] --> L3["03 Streams, big genomic files,<br/>grep & regex"]
  end
  C --> S
  subgraph S["Scripting"]
    direction TB
    L4["04 Shell scripts"] --> L5["05 R & Python 1"] --> L6["06 R & Python 2"] --> L7["07 Tidyverse<br/>& ggplot2"]
  end
  S --> H["<b>08 Talapas</b><br/>HPC & SLURM"]
  H --> R
  subgraph R["Reproducibility"]
    direction TB
    L9["09 Quarto, LaTeX,<br/>AI assistants, Git"] --> L10["10 Git, GitHub,<br/>GitHub Pages"]
  end
```
...
One small, real dataset runs through the whole term, so every new tool is used on data you already know.

::::: columns
::: {.column width="55%"}
-   **80 threespine stickleback** from the Cresko Lab, gut RNA-seq
-   **2 host genotypes** (A, B) x **2 microbiota treatments** (conventional, mono) = 4 groups of 20 fish
-   **600 genes**, raw read counts; one row per fish, one column per gene
-   A plain-text, tab-separated file (`.tsv`), 160 KB
:::

::: {.column width="45%"}
Download it in Part 2, once you have a shell and a project folder:
...
::: callout-tip
Week 2: count its rows. Week 3: `grep` a group out of it. Week 4: `awk` a mean. Weeks 5-7: load, wrangle and plot it in R and Python. Week 8: run it on Talapas. Week 9: write it up in Quarto.
:::
...
![](images/Lec00_img002.svg){fig-align="center" width="60%"}
...
![Every file has an address: the chain of directories from the root down to the file](images/Lec00_img005.svg){fig-align="center" width="85%"}
...
![`..` always points one level up, so `cd ../..` climbs two levels](images/Lec00_img007.svg){fig-align="center" width="85%"}
...
::::: columns
::: {.column width="50%"}
![Normally output prints to the screen](images/Lec00_img008.svg){width="100%"}
:::

::: {.column width="50%"}
![`>` sends it to a file instead](images/Lec00_img009.svg){width="100%"}
:::
:::::
...
![Each command's output becomes the next command's input](images/Lec00_img010.svg){fig-align="center" width="85%"}
...
| Install **today**              | Why                                                        |
|--------------------------------|------------------------------------------------------------|
| A **Unix shell**               | Terminal (macOS/Linux) or WSL + Ubuntu (Windows) - Part 2  |
| **R** and **Python**           | The two languages we script in                             |
| **Positron**                   | The IDE we will use in class; **Quarto is bundled** with it |
...
::: callout-important
**In class I will demo Positron.** Everything also works in VS Code (same editor underneath) and, for R, in RStudio - use them if you prefer, but if you are new to all of this, follow along in Positron. The [Software](../Software.html) page has full instructions for every tool.
:::
...
::: callout-note
We'll use citations in Quarto in **Lecture 09** - a Zotero library makes that painless.
:::
...
-   Install **R and Python first** (previous slide) - Positron finds them automatically and lets you switch between them
...
## Positron vs. VS Code (and RStudio)
...
::: callout-tip
You don't have to choose forever. Positron and VS Code share a lot of muscle memory - learn one and you mostly know the other. **Class demos use Positron**; the book covers all three.
:::
...
Open a terminal (in Positron: ``Ctrl-` ``) and run:
...
-   Bookmark the course [Resources](../Resources.qmd) page - the online **course book** ([Unix Fundamentals](../book/chapters/04-unix-fundamentals.html) and [Reproducible Documents with Quarto](../book/chapters/05-quarto-documents.html)) goes deeper on today's topics
...
-   You'll use SSH keys to connect to **GitHub** (Lecture 10) and **Talapas** (Lecture 08)
