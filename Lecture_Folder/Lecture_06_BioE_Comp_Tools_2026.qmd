...
-   Take a look at it in the Environment pane of RStudio, or the **Data Explorer** in Positron (R and Python)
...
## Running example: the stickleback table as a data frame

::: panel-tabset
### R

```{r}
#| label: lec06-stickle-read-r
#| echo: true
#| eval: true

stickle <- read.delim("../Data_Folder/Stickle_RNAseq.tsv", check.names = FALSE)
dim(stickle)                          # 80 fish x 604 columns
stickle[1:4, 1:7]                     # first rows, first columns
...
```{python}
#| label: lec06-stickle-read-py
#| echo: true
#| eval: false

import pandas as pd
stickle = pd.read_csv("../Data_Folder/Stickle_RNAseq.tsv", sep="\t")
...
## Running example: a first figure

::: panel-tabset
### R

```{r}
#| label: fig-lec06-stickle-boxplot
#| fig-cap: "Gene1 counts by host genotype and microbiota treatment"
#| out-width: "60%"
#| echo: true
#| eval: true
...
```{python}
#| label: lec06-stickle-boxplot-py
#| echo: true
#| eval: false

import matplotlib.pyplot as plt
stickle.boxplot(column="Gene1", by=["Genotype", "Microbiota"])
plt.ylabel("Gene1 read count"); plt.show()
```
:::

::: callout-tip
`check.names = FALSE` keeps the column name `Geno&Micro` as it is in the file; without it R would rename it `Geno.Micro`. Either way, back-ticks let you refer to awkward names: `` stickle$`Geno&Micro` ``.
:::
