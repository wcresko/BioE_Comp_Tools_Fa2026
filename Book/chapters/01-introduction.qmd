...
## The Course Dataset {#sec-course-dataset}
...
You will count its rows at the shell (@sec-files-pipes), pull out groups with `grep`, `cut`, and `awk` (@sec-grep-regex), read it into R and Python (@sec-r-programming, @sec-python-programming), reshape and plot it (@sec-data-wrangling, @sec-data-viz), run the analysis as a batch job on Talapas (@sec-hpc-talapas), and write it up as a Quarto report (@sec-quarto-documents). Download it once into your course project folder:
...
Before diving into the technical content, you'll need to set up your computing environment. This section covers the operating-system level: getting a Unix shell on macOS, Linux, or Windows. In class we type shell commands in the terminal built into the Positron IDE, but the shell itself is the operating system's, and this section is about making sure it is there. The rest of the toolkit (R, Python, Positron with its bundled Quarto, and later TinyTeX) is installed in @sec-toolkit, which also has a verification checkpoint and a troubleshooting table; Git is installed in @sec-git-github.
...
::: {.callout-tip title="Optional: Homebrew"}
macOS has no built-in package manager for command-line software. **Homebrew** (<https://brew.sh>) fills that gap: once installed, `brew install <name>` fetches and installs tools such as `wget`, `tree`, or a newer `git` with one command, and `brew update && brew upgrade` keeps them current. Install it by pasting the single command shown on the Homebrew home page into Terminal (it needs the Xcode command-line tools, which you just installed). It is not required for this course, but later chapters mention it as one way to install Git (@sec-git-github), and you will find that most macOS instructions on the web assume you have it.
:::

### Linux Users {#sec-linux-users}

Linux systems include multiple terminal applications. Open your distribution's default terminal emulator, often called "Terminal" or "Console" (frequently `Ctrl-Alt-T`). Everything else in this chapter's setup is already present; the only additions you will need are the compiler toolchain and R itself, both of which come from your package manager (`sudo apt install build-essential gfortran r-base r-base-dev` on Ubuntu/Debian) as described in @sec-toolkit.
...
1. Right-click **Windows PowerShell** (or **Terminal**) in the Start menu and choose **Run as administrator**
2. Type `wsl --install` and press Enter. This installs WSL *and* Ubuntu; there is no separate Ubuntu download. (If Ubuntu does not appear after the restart, run `wsl --install -d Ubuntu`, or install **Ubuntu** from the Microsoft Store)
...
This book accompanies a one-credit, ten-week course that meets for an hour a week, preceded by a four-hour bootcamp. The chapters are arranged in the order the course visits them, so the book can be read straight through, and @fig-course-roadmap shows how the weeks group into five sections. The **bootcamp** (Chapters 1 through 5) sets up your laptop and gives a first look at everything. **Computers and shells** (Weeks 1 to 3) covers how a computer is put together and where computation can happen, then the Unix shell: paths, files, pipes, and searching large text files with `grep`. **Foundations of scripting** (Weeks 4 to 7) moves from shell scripts to R and Python, and on to wrangling and plotting data frames with the tidyverse. **High-performance computing** (Week 8) takes the same analyses to Talapas. **Reproducible science** (Weeks 9 and 10) closes with Quarto, LaTeX, working responsibly with AI assistants, and version control with Git and GitHub.

![The ten-week roadmap: the bootcamp, three weeks on computers and the shell, four on scripting, one on high-performance computing, and two on reproducible science.](../images/Chap01_roadmap.svg){#fig-course-roadmap fig-align="center" width="100%"}

Most class time is devoted to coding practice rather than lecture, so expect to follow along with examples during class, work through the exercises at the end of each chapter, and bring your laptop every week. Assessment is deliberately light for a one-credit course: two problem sets, each assigned two weeks before it is due, and a term project in which you process a dataset of your own on Talapas and write it up as a reproducible Quarto document, with a one-page proposal due partway through the term. The current dates, readings, and slides for each week are on the course website.[^schedule]

[^schedule]: The schedule lives at <https://wcresko.github.io/BioE_Comp_Tools_Fa2026/Schedule.html>; the book itself avoids dates so that it stays correct from one year to the next.

::: {.callout-important title="Learning Takes Practice"}
Computational skills develop through repeated practice. Don't be discouraged if concepts don't click immediately: everyone struggles at first, and the people who become fluent are simply the ones who kept typing. The key is persistence and a willingness to experiment.
:::
...
Next, @sec-toolkit installs the software you will use all term, and @sec-computing-at-uo maps the accounts, storage, and computing the University provides. With those in place, @sec-unix-fundamentals opens the terminal, and @sec-computer-systems builds a mental model of the machine behind it.
