...
# Load required modules (inside the script, with a version number)
module purge
module load R/4.3.3
...
### Talapas Helper Commands {#sec-talapas-helpers}

| Task | Command |
|:-----|:--------|
| Which PIRGs (accounts) am I in? | `groups` |
| Which GPUs exist and which are free? | `/packages/racs/bin/slurm-show-gpus` |
| Set my default account once | `echo 'export SBATCH_ACCOUNT=myPIRG' >> ~/.bash_profile` |
| Home and project usage | `df -h ~` / `du -sh /projects/myPIRG/$USER` |

: Talapas-specific helper commands (see @sec-connecting-talapas for access details) {#tbl-talapas-helpers}
