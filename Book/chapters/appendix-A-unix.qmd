...
This appendix provides a comprehensive reference for Unix/Linux commands covered throughout the book, chiefly in @sec-unix-fundamentals, @sec-files-pipes, @sec-grep-regex, and @sec-shell-scripting.
...
| `ls -F` | Mark directories with `/`, executables with `*` | `ls -F` |
...
| `sort -k` | Sort by column (use `-kN,N` to sort on that column only) | `sort -k2,2 -n data.tsv` |
| `sort -t` | Specify field delimiter | `sort -t',' -k3,3 data.csv` |
...
| `uniq -u` | Show only lines that occur once | `sort file \| uniq -u` |
| `cut -f` | Extract fields (tab-delimited); ranges allowed | `cut -f2-5 data.tsv` |
...
| `sed -i` | Edit file in place (Linux/WSL; overwrites the file, so test first) | `sed -i 's/old/new/g' file.txt` |
| `sed -i ''` | Edit file in place on macOS (BSD `sed` requires a backup suffix; `''` means none) | `sed -i '' 's/old/new/g' file.txt` |
...
| `join` | Join sorted files on common field | `join file1.txt file2.txt` |
| `join -1 -2` | Choose the key column in each file | `join -1 2 -2 1 file1.txt file2.txt` |
| `fold -w` | Wrap lines to a fixed width (`-w1` = one character per line) | `tr -d '\n' < seq.fa \| fold -w1 \| sort \| uniq -c` |
| `split` | Split file into pieces | `split -l 1000 large.txt` |
| `shuf` | Randomly shuffle lines (or sample n lines) | `shuf -n 100 reads.txt` |
...
| `<<` | Here-document (inline multi-line input up to a marker) | `sort << EOF` ... `EOF` |
| `1>` | Redirect stdout (explicit form of `>`) | `cmd 1> out.txt` |
...
| `gunzip -k` | Decompress, keeping the .gz file | `gunzip -k file.txt.gz` |
| `gunzip -c` | Decompress to stdout (same as `zcat`) | `gunzip -c data.gz \| head` |
...
| `uname -m` | Machine architecture (e.g. `x86_64`, `arm64`) | `uname -m` |
| `date` | Current date/time | `date` |
| `date +%Y-%m-%d` | Formatted date | `date +%Y-%m-%d` |
| `cal` | Display calendar | `cal` |
| `uptime` | System uptime | `uptime` |
| `df -h` | Disk space usage | `df -h` |
| `df -h ~` | Free space on the filesystem holding your home directory | `df -h ~` |
...
| `history` | Show previously run commands | `history \| tail -20` |
...
### Special Variables in Scripts {#sec-script-variables}

| Variable | Description |
|:---------|:------------|
| `$0` | Name of the script |
| `$1`, `$2`, ... `$9` | First, second, ... ninth command-line argument (`${10}` and up need braces) |
| `$#` | Number of arguments |
| `$@` | All arguments as separate words (quote it: `"$@"`) |
| `$*` | All arguments as one word |
| `$LINENO` | Current line number in the script (useful in error traps) |

: Special variables available inside shell scripts (@sec-cli-arguments) {#tbl-script-variables}
