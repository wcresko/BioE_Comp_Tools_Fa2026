...
![The parts of a shell command. The prompt is printed by the shell to show it is ready; you type the command, any options that modify its behavior, and the arguments it should operate on.](../images/Chap05_img001.svg){#fig-shell-command-anatomy fig-align="center" width="80%"}
...
![A computer's storage is a system of directories (folders) and files. The same folders you click through in Finder or File Explorer are what you will navigate from the shell.](../images/Chap05_img002.svg){#fig-file-structure fig-align="center" width="85%"}
...
![The Unix file hierarchy. Every file and directory on the system descends from the root directory `/`; user home directories sit under `/Users` (macOS) or `/home` (Linux).](../images/Chap05_img003.svg){#fig-unix-file-hierarchy fig-align="center" width="85%"}
...
![Understanding file system paths. An absolute path is built by following the directory tree from the root (`/`) down through each level to the file; the highlighted route spells out the path one directory at a time.](../images/Chap05_img004.svg){#fig-paths fig-align="center" width="85%"}
...
![Every directory contains two hidden entries: `.`, which points to the directory itself, and `..`, which points to its parent.](../images/Chap05_img005.svg){#fig-unix-dot-files fig-align="center" width="85%"}

![Navigating with `..`: `cd ..` follows the parent link up one level in the tree, and a path such as `../other_dir` goes up one level and then down into a sibling directory.](../images/Chap05_img006.svg){#fig-unix-dotdot-navigation fig-align="center" width="85%"}
