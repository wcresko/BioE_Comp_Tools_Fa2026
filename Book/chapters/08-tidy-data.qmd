...
::: {.callout-tip}
The pipe transforms `x |> f(y)` into `f(x, y)`. Think of it as "take this, then do that." This makes code read like a recipe: take data, then filter, then select, then summarize. In Positron, `Ctrl/Cmd + Shift + M` inserts a pipe (@sec-appendix-shortcuts).
:::
