...
## Running Example: The Stickleback Data {#sec-viz-stickle}
...
The base R boxplot in @sec-r-programming showed one gene. With the long-format data from @sec-data-wrangling, ggplot2 shows several genes in one figure: a boxplot layer, colour for the microbiota treatment, and `facet_wrap()` to give each gene its own panel with its own y-axis.

```{r}
#| label: ch14-stickle-prep
#| message: false
library(tidyverse)
stickle_long <- read_tsv("../data/Stickle_RNAseq.tsv", show_col_types = FALSE) |>
  pivot_longer(starts_with("Gene"), names_to = "gene", values_to = "count")
```

```{r}
#| label: fig-ch14-stickle-facets
#| fig-cap: "Read counts for four genes by host genotype and microbiota treatment. Each panel is one gene; colour is the microbiota treatment."
#| fig-width: 8
#| fig-height: 5
stickle_long |>
...
Nothing in the plotting code refers to a particular gene: change the `filter()` and the same figure describes any subset of the 600. That separation between the data and the description of the graphic is what the grammar of graphics buys you.
