...
![The Grammar of Graphics framework](../images/visualization_grammar_of_graphics.jpeg){#fig-grammar-graphics}
...
![A flow chart for choosing a plot type from the number and types of variables to display.](../images/chart_selection_flow_chart.svg){#fig-chart-selection width="90%"}
...
![Scatter plots are excellent for exploring relationships between two continuous variables. This example shows a positive correlation between two traits, demonstrating how scatter plots can reveal patterns in biological data.](../images/visualization_scatter_correlation.jpeg){#fig-scatter-correlation width="60%"}
...
![Anatomy of a box plot](../images/visualization_boxplot_anatomy.jpeg){#fig-boxplot-anatomy width="60%"}
...
![Different curve-fitting methods send different messages about your data. Choose methods that match your hypotheses and data characteristics, not just what makes the prettiest curve. Source: xkcd.com](../images/visualization_curve_fitting.png){#fig-curve-fitting-humor width="70%"}
:::
...
![A "ticker tape" of unrelated values plotted side by side. There is no question this chart answers: the categories have no order, the lines imply a trend that does not exist, and the reader cannot compare anything.](../images/bad_graphic_ticker_tape.jpeg){#fig-bad-ticker-tape width="75%"}

![A line to no understanding: lines connect points that are not sequential observations, so the slopes between them mean nothing.](../images/bad_graphic_line_to_nowhere.jpeg){#fig-bad-line width="75%"}

![A bake sale of pie charts. Comparing angles within one pie is hard; comparing angles across many pies is nearly impossible. A single faceted bar chart or dot plot would show every comparison at once.](../images/bad_graphic_pie_chart_bake_sale.jpeg){#fig-bad-pies width="75%"}
...
![As broadcast: the y-axis range is chosen so a small change looks enormous](../images/bad_graphic_truncated_y_axis_news.jpeg){#fig-truncated-news}

![The same data with an honest axis](../images/good_graphic_unemployment_line_plot.jpeg){#fig-truncated-fixed}
...
![Minard's 1869 map of Napoleon's Russian campaign. Source: Charles Joseph Minard, 1869.](../images/minard_napoleon_russian_campaign.jpeg){#fig-minard width="90%"}
...
![A modern recreation of Minard's design applied to a different dataset. Source: FiveThirtyEight.](../images/minard_style_modern_recreation.jpeg){#fig-minard-modern width="90%"}
