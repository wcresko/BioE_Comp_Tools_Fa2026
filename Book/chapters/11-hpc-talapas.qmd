...
![Talapas cluster architecture: SSH into one of four login nodes, submit work to the SLURM scheduler with `sbatch` or `srun`, and it runs on CPU, GPU, or high-memory ("fat") compute nodes that all share the same storage](../images/appendix_F_S03_talapas_architecture.svg){#fig-talapas-architecture fig-align="center" width="85%"}
...
flowchart LR
    ...
    SC --> SCP["/scratch/PIRG<br/>Working data, purged after 90 days"]
    ...
    style SCP fill:#ffa07a,stroke:#333,stroke-width:2px
```
...
![SLURM job lifecycle: `sbatch` places the job in the queue, it waits for resources, runs on a compute node, and leaves its output in files](../images/appendix_F_S10_slurm_workflow.svg){#fig-slurm-workflow fig-align="center" width="100%"}
