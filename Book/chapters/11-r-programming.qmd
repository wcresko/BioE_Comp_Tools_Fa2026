...
- Use Positron or RStudio as an integrated development environment for R
...
- Manage the working directory by working in a project folder (or an RStudio Project)
...
## Positron and RStudio: Your R Environment {#sec-rstudio}

While R can run from the command line, most users work in an integrated development environment (IDE). In this course that is mainly **Positron** (@sec-positron), which is built on the same core as VS Code and adds the panes a data scientist needs; **RStudio** (@sec-rstudio-overview) offers the same panes in a fixed four-pane layout, and everything below applies to it with the pane names noted.

### The IDE Interface {#sec-the-rstudio-interface}

Positron organizes your workspace into a few key regions (RStudio's equivalents are in parentheses):

**Editor (center; RStudio: Source, top-left).** Where you write and edit scripts and Quarto documents. Code here isn't executed until you explicitly run it.

**Console (bottom; RStudio: bottom-left).** The interactive R (or Python) session. Commands typed here execute immediately, and lines sent from the editor appear here with their output.

**Variables (right; RStudio: Environment/History, top-right).** Shows the objects you've created in the current session, with their type and size; click a data frame to open it in the **Data Explorer**.

**Plots, Help, and Explorer (right and left; RStudio: Files/Plots/Packages/Help, bottom-right).** The plot viewer, help documentation, and the file browser for the folder you have open.

::: {.callout-note title="R is not the IDE"}
`R` is the programming language; Positron, RStudio, or VS Code is the integrated development environment that lets you write and run R (and Python) comfortably. R must be installed first, and the IDE then finds it. If you ever need to, you can run R directly from the shell by typing `R`.
:::
...
### Project Folders, RStudio Projects, and the Working Directory {#sec-rstudio-projects}
...
RStudio does the same thing with **RStudio Projects**: File → New Project → New Directory (or Existing Directory) creates a `.Rproj` file, and opening that file sets the working directory to the project folder.
...
: R data types. The abbreviations are what tibbles (@sec-tibbles) and `glimpse()` print under each column name. {#tbl-data-types}
...
::: {.callout-note}
Package names are case-sensitive and must be in quotation marks when installing. You only need to install a package once on your system (and again after upgrading R). Positron offers the same thing through the Command Palette (Cmd/Ctrl-Shift-P → **R: Install Packages**) and RStudio through *Tools → Install Packages*; `installed.packages()` (or RStudio's Packages tab) lists what is installed. Bioconductor packages are installed with `BiocManager::install()` instead (@sec-install-checklist).
:::
...
::: {.callout-note}
Row names are a base R convenience. The tidyverse's tibbles (@sec-tibbles) drop them, on the principle that a sample identifier is data and belongs in an ordinary column such as `sample_id`; `tibble::rownames_to_column()` makes the conversion. In this book we keep identifiers in columns.
:::
...
In Positron or RStudio, you can also use `View()` to open a spreadsheet-like viewer (Positron's Data Explorer) for interactive exploration:

```{r}
#| label: view-df
#| eval: false

# Opens data in a viewer pane (Positron or RStudio)
View(experiment)
```
...
Reading a file rarely fails loudly; it fails quietly by giving you the wrong column types. Immediately after any import, look at the result with `str()` or `glimpse()`, `head()`, `dim()`, and `summary()` (or `View()` in Positron or RStudio). A numeric column that shows up as `character` means a non-numeric entry is hiding somewhere in the file.
:::
...
::: {.callout-tip}
The tidyverse packages (covered in @sec-data-wrangling) provide more intuitive alternatives like `group_by()` and `summarize()` for this workflow.
:::
...
## Running Example: The Stickleback Data {#sec-r-stickle}
...
The whole table fits comfortably in an R data frame. `read.delim()` reads tab-separated files with a header; `check.names = FALSE` keeps the column name `Geno&Micro` exactly as it is in the file (otherwise R would rename it `Geno.Micro`), and backticks let you refer to such a name later.

```{r}
#| label: ch11-stickle-read
stickle <- read.delim("../data/Stickle_RNAseq.tsv", check.names = FALSE)
dim(stickle)                          # 80 fish x 604 columns
stickle[1:4, 1:7]                     # the first rows and columns
table(stickle$Genotype, stickle$Microbiota)
summary(stickle$Gene1)
```

A first figure takes one line of base R. The formula `Gene1 ~ Genotype + Microbiota` asks for one box per combination of the two factors:

```{r}
#| label: fig-ch11-stickle-boxplot
#| fig-cap: "Gene1 read counts by host genotype and microbiota treatment, drawn with base R's boxplot()."
boxplot(Gene1 ~ Genotype + Microbiota, data = stickle,
        col = c("steelblue", "tomato", "steelblue", "tomato"),
        ylab = "Gene1 read count", las = 2)
```

Genotype A fish have far higher Gene1 counts than genotype B, and within each genotype the mono-associated fish sit a little higher, the same pattern the `awk` script in @sec-shell-scripting found. @sec-data-wrangling redoes this with the tidyverse for all 600 genes at once, and @sec-data-viz turns the figure into publication-quality ggplot2.
...
- Positron and RStudio provide integrated development environments with a console, variables/environment pane, plots pane, and data viewer
...
These fundamentals prepare you for more advanced R programming, including the tidyverse packages covered in @sec-data-wrangling and @sec-data-viz. A command reference is in @sec-appendix-r.
...
**Exercise 8: Exploring Your IDE and the Help System**

1. Locate the editor, Console, Variables (Environment), and Plots panes; open the Help pane (in RStudio, click through the Packages and Help tabs in the bottom-right pane)
