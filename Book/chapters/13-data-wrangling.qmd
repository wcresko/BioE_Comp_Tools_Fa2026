# Data Wrangling with the Tidyverse {#sec-data-wrangling}

```{r}
#| label: ch13-wrangling-setup
#| include: false
library(tidyverse)
library(gt)
...
@sec-tidy-data laid out what well-organized data looks like. This chapter is about making data look that way, and then asking questions of it, in R. The **tidyverse** is a family of packages built around tidy data and a shared grammar: read a file with **readr**, pick and transform rows and columns with **dplyr**, reshape with **tidyr**, and pass the result from one step to the next with the pipe. Everything here assumes the R fundamentals from @sec-r-programming (vectors, data frames, functions) and pairs naturally with the plotting in @sec-data-viz.
...
![The pipe operator passes the result of each step to the next, turning a nest of function calls into a readable sequence.](../images/Chap11_img004.jpeg){#fig-pipe-operator width="80%"}
...
::: {.callout-tip}
The pipe transforms `x |> f(y)` into `f(x, y)`. Think of it as "take this, then do that." This makes code read like a recipe: take data, then filter, then select, then summarize. In Positron and RStudio, `Ctrl/Cmd + Shift + M` inserts a pipe (@sec-appendix-shortcuts).
:::
...
Conditions are built from the comparison and logical operators in @tbl-filter-operators (see @sec-logical-operators-and-comparisons). Separate conditions with commas for AND; `&` and `|` combine conditions the way set intersection and union do (@fig-boolean-venn).
...
![Boolean operators as set operations: `&` keeps rows in both sets, `|` keeps rows in either, and `!` keeps the complement.](../images/Chap11_img005.svg){#fig-boolean-venn width="60%"}
...
![Tibbles are the tidyverse's enhanced data frames: they print compactly, show each column's type, and never change the type of what you give them.](../images/Chap11_img006.jpeg){#fig-tibbles width="80%"}
...
## Running Example: The Stickleback Data {#sec-wrangling-stickle}

::: {.callout-note title="Running example"}
This section applies the chapter to the course dataset described in @sec-course-dataset (80 stickleback, 4 treatment groups, 600 genes). The same steps appear on the Week 7 slides, so the book and the lecture can be read side by side.
:::

The stickleback table is wide: one row per fish, 600 gene columns. That is convenient for looking at one gene, and awkward for asking a question of every gene at once. `pivot_longer()` turns the gene columns into two: the gene's name and its count, giving one row per fish-gene pair, 48,000 rows in all. The awkward column name is fixed once, with `rename()`, on the way in. Because this book renders data frames as full tables, the chunk shows only the first six rows; printing all 48,000 would produce a table too large for the page (and, as it happens, for Pandoc).

```{r}
#| label: ch13-stickle-long
#| message: false
library(tidyverse)
stickle <- read_tsv("../data/Stickle_RNAseq.tsv", show_col_types = FALSE) |>
  rename(geno_micro = `Geno&Micro`)

stickle_long <- stickle |>
  pivot_longer(starts_with("Gene"), names_to = "gene", values_to = "count")
dim(stickle_long)                 # 48,000 rows: one per fish x gene
slice_head(stickle_long, n = 6)   # the first few (never print all 48,000!)
```

With the data in long form, a grouped summary computes the mean count of every gene in every treatment group in one pipeline, and `filter()` pulls out whichever gene you are curious about:

```{r}
#| label: ch13-stickle-means
...
These are the same four numbers that the `awk` script in @sec-shell-scripting printed, now for all 600 genes in a single data frame that can be sorted, filtered, joined to annotation, or handed to ggplot2 (@sec-data-viz). That is the payoff of tidy, long-format data: one reshaping step, and every question about every gene becomes a `group_by()` away.

## Summary {#sec-wrangling-summary}
...
## Additional Reading {#sec-wrangling-additional-reading .unnumbered}

To deepen your understanding of the tidyverse:
...
## Exercises {#sec-wrangling-exercises .unnumbered}

::: {.callout-tip title="Practice Exercises"}
**Exercise 1: Reshape Data**
